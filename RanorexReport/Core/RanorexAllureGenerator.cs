...
namespace RanorexReport.Core
{
    public static class RanorexAllureGenerator
    {
        ...
        static string ranorexPath;
        static string ranorexZipFile;

        static string zipFile = string.Empty;
        static string pdfFile = string.Empty;
        static string allureResultsPath;
        static string? buildVersion;

        public static void Generate(
            string ranorexArtifactsPath,
            string allureOutputPath,
            int? buildId,
            string? runName,
            string? zipName)
        {
            ranorexPath = ranorexArtifactsPath;
            ranorexZipFile = Path.Combine(ranorexPath, zipName);
            allureResultsPath = Path.Combine(allureOutputPath, "allure-results");
            ...
            File.Copy(
                "categories.json",
                Path.Combine(allureResultsPath, "categories.json"),
                overwrite: true);

            if (!string.IsNullOrEmpty(buildVersion))
            {
                Directory.CreateDirectory(Path.Combine(allureResultsPath, $"{buildVersion}_A3_Run_{buildId}"));

                CreateExecutorFile($"{buildVersion}_A3_Run_{buildId}", buildId.Value);
            }
            else
            {
                CreateExecutorFile(runName, buildId.Value);
            }
        }
